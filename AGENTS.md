...
- backend 代码改动后先同步 `backend/.planning/codebase/`，再按影响更新根级系统文档；文档变更至少运行 `git diff --check`。
- 测试采用可执行 assert 脚本；真实模型、MinerU、Oracle 或外部 HTTP 测试必须与普通本地回归分开。
...
- backend 事实来源：[backend/.planning/codebase/](backend/.planning/codebase/)，实现细节以其中 7 份 fact docs 为准。
