...
- **MiniMax 凭据现已单源、无复制（改进）**：经 commit a30bb99（"切换MiniMax适配为Anthropic兼容协议"）与 9c78cf2（"remove fallback logic for minimax model config"）后，`DeepAgentsBrainFactory.__init__`（harness.py）在 `model is None` 时执行 `init_chat_model(f"anthropic:{os.getenv('MINIMAX_MODEL')}", api_key=os.getenv("MINIMAX_API_KEY"), base_url=os.getenv("MINIMAX_BASE_URL"), thinking={"type": "adaptive"})`，即落到 LangChain `ChatAnthropic`。仅通过 `os.getenv` 读取 `MINIMAX_MODEL`/`MINIMAX_API_KEY`/`MINIMAX_BASE_URL` 三个变量，**无默认值、无 fallback、无复制到 `OPENAI_API_KEY`/`OPENAI_API_BASE`**；thinking 固定启用为 `adaptive`。相较旧的"复制为 OPENAI_* + ANTHROPIC_* fallback"实现，环境变量泄漏面**减小**——凭据不再以两套前缀重复出现在进程环境中。
