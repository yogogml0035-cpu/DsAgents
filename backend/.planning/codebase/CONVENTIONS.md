...
| 场景 | 入口 |
| --- | --- |
| HTTP 提交 run | `POST /runs`（body `{message, session_id?}`），轮询 `GET /runs/{run_id}` |
| 自检 / 主要验证 | `python backend/self_check.py`（结尾打印 `self-check passed`） |
| 启动 HTTP 服务 | `cd backend && uv run uvicorn api:app --host 0.0.0.0 --port 8000` |
| 程序内调用 | `AgentResources(config)` + `create_harness(resources).execute_run(message, session_id, run_id)` |
...
- **真实错误透传**：见 §5。
- **优先删减范围**：HTTP 表面只保留 `POST /runs` / `GET /runs/{run_id}` / `POST /files`（旧 session 端点已删，见 §7）。
...
- fail-fast 模式：`parse_document` 在缺 `MINERU_*` 环境变量时 `raise RuntimeError("Missing required environment variable: ...")`（自检显式断言此行为）。
...
- 失败 run **不**回滚 thread；下一次同 `session_id` 继续用同一 `thread_id`（自检 `_FakeBrain` 用 `threads` dict 按序号验证此行为）。

## 7. HTTP 表面（已确认）

- 当前只保留：`POST /runs`、`GET /runs/{run_id}`（可选 `?after_event_id=` 取增量）、`POST /files`。
...
- Brain 调用统一走 `brain.stream({"messages":[{"role":"user","content":message}]}, config={"configurable":{"thread_id":session_id}}, stream_mode=["messages","custom","values"], version="v2")`。
- payload **只**传当前 user message（自检 `_FakeBrain` 显式断言 `len(messages)==1` 且无 `id` 字段，证明不再注入 `RemoveMessage`）。
- run ledger 事件类型固定 5 种：`status` / `thinking` / `text_delta` / `tool_status` / `values`。
- 工具状态中间件只发 `started` / `completed` / `error` 三态（自检断言）。
...
- 自检用 `_FakeBrainFactory` / `_FakeBrain`：验证 Brain 可替换、payload 只含当前 user message、`thread_id` 路由、失败 run 后同 thread 续跑不回滚。
- 自检**结尾必须**打印 `self-check passed`（CI/手工都以此字符串判定通过）。
