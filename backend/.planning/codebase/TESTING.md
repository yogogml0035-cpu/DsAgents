...
| `tests.test_harness` | stream 归一化（7 类事件，ToolMessage 和 telemetry 工具返回不泄漏为文本；WGQ/DK 仅终态摘要且无 schema 草稿）、cancel/`GraphDrained`、`NoProgress`、ToolTelemetry、MemoryMiddleware、**StructuredOutputRecovery**（两 schema、重试封顶、`jump_to: "end"`、空壳 all-null `input_problems` / 其它失败耗尽无 structured_response）、WGQ/DK 与普通 Tecan finalizer 投影 | `FakeBrainFactory`、`create_agent` 小图、`patch` env；临时 resources |
