...
- `tests/` 下有 `__init__.py`，使 `tests` 成为可被 `python -m tests.test_xxx` 导入的包；**不**用 `python backend/tests/test_xxx.py`（绝对包内导入 `from dsagents... import ...` 会失败）。
...
- 网络/环境替身：保留 MinerU stream/requests patch；业务测试 patch `dsagents.integrations.artifacts.artifacts_root`，Philips 用空环境验证 Oracle fallback。
...
- **代码变更**（`dsagents/**/*.py` 或 `backend/tests/*.py`）：按影响范围跑对应 `cd backend && python -m tests.test_xxx`。
...
- **真实集成验证（手动、非默认）**：先 `uvicorn dsagents.api:app --port 8500`（cache baseline 用 8000），再 `python -m tests.test_real_image_run` / `test_real_multi_pdf_run` / `test_minimax_cache_baseline`，并按需设置 `DSAGENTS_RUN_REAL_*_TEST=1` / `MINIMAX_API_KEY` / `DSAGENTS_API_BASE_URL` / `DSAGENTS_BASE_URL` / `DSAGENTS_IMAGE_PATH` / `DSAGENTS_PDF_DIR`。
