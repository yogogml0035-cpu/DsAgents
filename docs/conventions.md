...
- **run 是事件源**：`run_events` 表 append-only 存完整规范化事件与 raw chunk；`runs` 表是事件投影出的快照。`values` snapshot 只保留在 raw 中，外部消费规范化事件。短期上下文不再自建回放，统一交给 LangGraph `checkpointer` + `thread_id=session_id`。（旧 `session` 模块/表/事件回放已在 commit `8890292` 移除。）
