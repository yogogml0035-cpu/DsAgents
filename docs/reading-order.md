...
| **改 Tecan Skill / Excel** | kebab 资源 `backend/skills/tecan-import/{SKILL.md,references/,assets/}` + 可 import 包 `backend/skills/tecanimport/scripts/{tools.py,documents.py}` 与 `tests/test_tecan_import.py`；改资源时同步 `pyproject.toml` package-data |
