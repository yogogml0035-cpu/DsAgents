...
- 工具：静态 5 个，覆盖 PDF/ZIP 通用处理、WGQ / DK 共享的 12NC 主数据、共享 XLSX inspection 和 Tecan finalizer。两个 workflow 用同一 denylist 排除 Tecan finalizer、保留共享 lookup；禁止业务-only allowlist。没有 Tecan extractor SubAgent 或业务状态机。
- middleware：集中于 `runtime/middleware.py`。WGQ / DK 均使用各自 schema 的有界 `StructuredOutputRecovery`（`can_jump_to` 含 `"end"`，耗尽显式 `jump_to: "end"`）；普通 run 不强制 schema。两个 workflow 均以共享 runtime finalizer 验证终态。
